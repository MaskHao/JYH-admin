<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="总人数" prop="GoodStock"/>
      <el-table-column align="center" label="报名费" prop="GoodPrice"/>
      <el-table-column align="center" label="赠送积分" prop="GoodPointPrice"/>
      ...
      <el-table-column align="center" label="操作">
        <template slot-scope="scope">
          ...
          >修改</el-button>
          <el-tooltip class="item" effect="dark" content="迁移到活动回顾中" placement="top-end">
            <el-button
              class="handle-btn"
              type="primary"
              size="medium"
...
<script>
import { activePage, activeDelete, upcomingConvertToLove } from '@/api/action'
...
export default {
  ...
  methods: {
    getList({ page, limit } = {}) {
      ...
      activePage(this.listQuery).then(res => {
        if (res.Result === 1) {
    ...
    // 活动详情
    actionInfo(GoodId) {
      this.$router.push('/love_notices/ln_detail/' + GoodId)
    },
    // 上架
    onaction(row, GoodId) {
      prodOnOff({
    ...
    // 修改活动
    updateAction(GoodId) {
      this.$router.push('/love_notices/ln_update/' + GoodId)
    },
